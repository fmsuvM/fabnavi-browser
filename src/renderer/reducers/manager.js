...
import {
    ...
    SEARCH_RELATED_PROJECTS_RESULT
...
const updateRelatedProjects = (projects, tag, obj) => {
    const _obj = obj;
    _obj[`${tag}`] = projects;
    return _obj;
};
...
export default handleActions(
    {
        ...
        [SEARCH_RELATED_PROJECTS_RESULT]: (state, action) => {
            const{ data } = action.payload;

            return {
                ...state,
                isFetching: false,
                relatedProjects: updateRelatedProjects(data, state.searchQuery, state.relatedProjects)
            };
        }
    },
    initialState
